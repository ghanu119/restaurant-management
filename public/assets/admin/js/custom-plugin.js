...
function actionBtnHtml ( data, isViewLink = false ) {
    var $edit = $("<a/>", {
        html : "<i class='fa " + (isViewLink ? 'fa-eye':'fa-edit') +" text-primary' title='" + (isViewLink ? 'View':'Edit') +"'>",
        href : serverUrl + data.slug + "/" + data.id + "/" + (isViewLink ? 'view':'edit') 
    })[0].outerHTML
    var $delete = '';
    ...
    if( data.view_class ){
        ...
        $view = $("<a/>", {
            html : "<i class='fa fa-trash text-primary' title='View'>",
            href : "#",
            class : "text-red " + data.view_class,
            "data-id": data.id
        })[0].outerHTML;
    }
    if( $view !== '' ){
